...
public class UltimateTabuleiro {
  ...
  public UltimateTabuleiro copy() {
    UltimateTabuleiro copia = new UltimateTabuleiro();
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        copia.tabuleiros[i][j] = this.tabuleiros[i][j].copy();
      }
    }
    return copia;
  }
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        for (int k = 0; k < 3; k++) {
          ...
          // percorrer cada linha(i) do ultimate ir aos 3 tabuleiro (k) dar print na mesma
          // linha (j)
          if (k != 2)
            sb.append(" ");
        }
