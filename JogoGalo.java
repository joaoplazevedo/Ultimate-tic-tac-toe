...
public class JogoGalo {
    ...
    private static void RandomAi(UltimateTabuleiro t) {
        ...
        List<int[]> moves;
        if (forcedPlay == -1) {
            moves = t.getValidMoves(-1);
            idx = r.nextInt(moves.size());
        } else {
            moves = t.getValidMoves(forcedPlay);
            idx = r.nextInt(moves.size());
        }
    ...
    private static int[] readInput(UltimateTabuleiro t) {
        ...
        sc.close(); // eu ainda nao consegui perceber o problema de nao fechar, mas fica aqui que eu
                    // nao quero ver o aviso de problema
