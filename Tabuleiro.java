public class Tabuleiro {
    ...
    public boolean validPlay(int i, int j) {
        if (i < 0 || j < 0 || i > 2 || j > 2 || t[i][j] != '-') return false;
        return true;
    }
    ...
    public String toString() {
        StringBuilder str = new StringBuilder(); // ganhar o habito de usar esta classe para ser mais eficiente no uso de memória
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                str.append(t[i][j]);
            }
    ...
    public char checkEnd() {
        if (moveCount == 0) return '-'; // ainda nao houve nenhum movimento feito
        if (
